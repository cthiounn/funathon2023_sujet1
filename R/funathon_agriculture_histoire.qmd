...
Vous pourrez faire le pont avec les données de 2020 pour tenter une comparaison.
...
### Création de la colonne code département

surfaces_ra1852_dep <- surfaces_ra1852 %>%
  mutate(code_dep = ifelse(nchar(as.character(code))==2, paste0("0", substr(as.character(code),1,1)), substr(as.character(code),1,2)))


### Création de la fonction qui permet d'ajouter les coordonnées du centre

ajout_coord <- function(nom, code_dep){
  nom <- str_replace_all(nom," +","%20")
  url_dep = "https://geo.api.gouv.fr/communes?nom=%s&codeDepartement=%s&fields=centre&boost=population&limit=1"
  url_dep_2 <- sprintf(url_dep, nom, code_dep)
  trouve <- is.null(fromJSON(url_dep_2)$centre$coordinates)
  if(!trouve){
    return(fromJSON(url_dep_2)$centre$coordinates[1])
}else{
    return(NA)
  }
}

### Ajout de la colonne coordonnées du centre
surfaces_ra1852_code_dep <- surfaces_ra1852_dep %>%
  rowwise() %>%
  mutate(centre = ajout_coord(nom = canton, code_dep = code_dep)) %>%
  ungroup()
...
Il se peut que des coordonnées de chefs-lieux ne soient pas trouvées.
Identifiez-les et corrigez-les en mettant à jour la colonne centre avec les vecteurs de coordonnées de la ville recherchée.
...
pb_coord <- surfaces_ra1852_code_dep %>%
  filter(is.na(centre) & !(is.na(departement)))

surfaces_ra1852_coordonnees_centre_corrigees <- surfaces_ra1852_code_dep %>%
  mutate( centre =
    case_when(
...
surfaces_ra1852_lat_long <- surfaces_ra1852_coordonnees_centre_corrigees %>%
  mutate(latitude = ifelse(is.na(centre), NA, unlist(centre[2])),
         longitude = ifelse(is.na(centre), NA, unlist(centre[1])))
...
-   nous avons une surface géographique que l'on souhaite découper en zone (des cantons).
    ...
    Ce n'est pas forcément le centre mais en tant que chef-lieu, nous considérons que sa zone d'influence l'entoure.
